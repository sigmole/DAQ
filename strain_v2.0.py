...
# 警告：客户端关闭后未释放socket对应的receivedata线程对象
...
class ClientConnect(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            print("client socket:" + str(self.client_socket))
            print("addres_client:" + str(self.address_client))
            receive_thread = ReceiveData(self.client_socket, self.address_client)
            receive_thread.start()                                                                                      #反复start的问题
            receive_thread.setPriority(QThread.NormalPriority)
            client_pool.append(self.client_socket)
            print(receive_thread)
            self.client_num = len(client_pool)
    ...
    def run(self):
        ...
        while self.using:
            ...
            try:
                ...
                print(f"Client forcibly closed the connection.")
                self.client_socket.close()
                client_pool.remove(self.client_socket)
                self.using = 0
                self.client_num = len(client_pool)
                window.label_4.setText("已连接%d个客户端" % self.client_num)
                break
